...
library(pROC)
library(table1) # to create frequencies tables
...
vis_dat(d1) # double check after remove all NA
```
...
# double-check after make changes
table(d3$Path1)
table(d3$Action1)
table(d3$Hemisphere)
...
table(d4$Hemisphere) # double-check and see that the level Unspecified is removed
```
...
a1 <- ggplot(d4, aes(gg2c4, CasesAll)) + geom_boxplot()
a2 <- ggplot(d4, aes(gg2c4, Deaths)) + geom_boxplot()
a3 <- ggplot(d4, aes(gg2c4, Hospitalizations)) + geom_boxplot()
a4 <- ggplot(d4, aes(gg2c4, MeanD1)) + geom_boxplot()
a5 <- ggplot(d4, aes(gg2c4, MeanI1)) + geom_boxplot()
a6 <- ggplot(d4, aes(gg2c4, MedianD1)) + geom_boxplot()
a7 <- ggplot(d4, aes(gg2c4, MedianI1)) + geom_boxplot()
a8 <- ggplot(d4, aes(gg2c4, OBYear)) + geom_boxplot()
a9 <- ggplot(d4, aes(gg2c4, RiskAll)) + geom_boxplot() 



#you can use the facet_wrap functionality in ggplot for it, or do it some other way.

gridExtra::grid.arrange(a1, a2, a3, a4, a5, a6, a7, a8, a9)
```
...
b1 <- ggplot(data=d4, aes(Action1, gg2c4)) + geom_count()
b2 <- ggplot(data=d4, aes(Country, gg2c4)) + geom_count()
b3 <- ggplot(data=d4, aes(Hemisphere, gg2c4)) + geom_count()
b4 <- ggplot(data=d4, aes(Path1, gg2c4)) + geom_count()
b5 <- ggplot(data=d4, aes(season, gg2c4)) + geom_count()
b6 <- ggplot(data=d4, aes(Trans1, gg2c4)) + geom_count()+ theme(axis.text.x=element_text(angle=45, hjust=1, vjust=0.9), text=element_text(size=9))
b7 <- ggplot(data=d4, aes(Vomit, gg2c4)) + geom_count()
b8 <- ggplot(data=d4, aes(Setting, gg2c4)) + geom_count()

gridExtra::grid.arrange(b1,b2,b3,b4, ncol=2)
gridExtra::grid.arrange(b5,b6,b7,b8, ncol=2)
```
...
table1(~ Action1+Country+Hemisphere+Path1+season+Trans1 |gg2c4, data=d4)
```
...
d5 <- d4 %>%
  filter(Action1 != 'No') %>%
  mutate(Country =fct_collapse(Country,Other=c('Other','Multiple','Unspecified')),
    Trans1=fct_collapse(Trans1,Environmental=c('Environmental','Waterborne')),
    season= fct_recode(season, "Unknown"=""))%>% 
  droplevels() %>%
...
predvalue <- rep('No', length(d5$gg2c4))
measureACC(d5$gg2c4, predvalue)
...
for (nn in 1:npred){
    unidata = data.frame(gg2c4 = outcome, d5[,nn+1] )
...
mytask = makeClassifTask(id='fullanalysis', data = d5, target = outcomename, positive = "Yes")
...
predvalue <- rep(0, length(d5$gg2c4))

truth <- d5 %>% mutate(gg2c4= ifelse(gg2c4=='Yes',1,0))
roc(truth$gg2c4, predvalue)
...
```{r roc-plot}
set.seed(1111) #makes each code block reproducible
d6 <- d5 %>% dplyr::select(gg2c4, sfeatga_res$x )
mytask = makeClassifTask(id='rocanalysis', data = d6, target = outcomename, positive = "Yes")
...
mytask = makeClassifTask(id='fullanalysis', data = d5, target = outcomename, positive = "Yes")
fullmodel = resample(mylearner, task = mytask, resampling = sampling_choice, show.info = FALSE, measures = mlr::auc )
AUC_lda_fullmodel = fullmodel$aggr[1]
print(AUC_lda_fullmodel)
...
set.seed(1111) #makes each code block reproducible
d7 <- d5 %>% dplyr::select(gg2c4, sfeatga_res$x )
mytask = makeClassifTask(id='rocanalysis', data = d7, target = outcomename, positive = "Yes")
lda_mod = train(mylearner, mytask)
lda_pred = predict(lda_mod, task = mytask)
df1 = generateThreshVsPerfData(list(logistic = log_pred, LDA = lda_pred ), measures = list(fpr, tpr))
